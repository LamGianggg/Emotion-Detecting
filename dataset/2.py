...
# ================= CONFIG =================
FOLDER = "D:/UserData/Downloads/archive/Voice Emotion Dataset/disgust"
OUTPUT_CSV = "disgust.csv"
ERROR_LOG = "error_log.txt"


# ================= SAFE WRITE (ANTI-CRASH) =================
def safe_write_csv(output_csv, fieldnames, row, header_written):
    for _ in range(5):  # retry 5 lần
        try:
            ...
            return True, header_written

        except PermissionError:
            print("⚠ File đang bị khóa (Excel?), retry...")
            time.sleep(1)

    return False, header_written
...
def process_folder(folder_path, output_csv, error_log):
    ...
    for idx, file in enumerate(files):
        ...
        if isinstance(result, dict):
            ok, header_written = safe_write_csv(
                output_csv,
                result.keys(),
                result,
                header_written
